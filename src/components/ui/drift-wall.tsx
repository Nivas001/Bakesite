  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <span className="drift-wall__inner">
        <img
          src={item.image || "/hero/hero-3d-cookie.jpg"}
          alt={item.title ?? ""}
          loading="lazy"
          decoding="async"
          draggable={false}
        ...
        {item.title && <span className="drift-wall__label">{item.title}</span>}
      </span>
    );
    const commonProps = {
    ...
    if (item.href) {
      if (item.href.startsWith("/")) {
        return (
          <Link key={id} to={item.href} {...commonProps}>
            {inner}
          </Link>
        );
      }
