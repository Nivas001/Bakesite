...
export function Cake3dModelViewer() {
  ...
  useEffect(() => {
    ...
    // Key Light (warm gold)
    const keyLight = new THREE.DirectionalLight(0xffd59e, 2.8);
    ...
    const fillLight = new THREE.DirectionalLight(0xffc2d1, 1.4);
  ...
  useEffect(() => {
    ...
    if (lightingPreset === "warm") {
      sceneRef.current.background = new THREE.Color(0x180f0a);
      ...
      sceneRef.current.background = new THREE.Color(0x141416);
      lights.children.forEach((l) => {
      ...
      sceneRef.current.background = new THREE.Color(0x0a0508);
      lights.children.forEach((l) => {
  ...
  return (
    <div id="3d-cake-studio" className="w-full scroll-mt-24">
      <div className="relative rounded-[2.5rem] border-[3.5px] border-[#2C1810] bg-gradient-to-b from-[#180E08] via-[#24140B] to-[#120804] text-white p-4 sm:p-8 lg:p-10 shadow-2xl overflow-hidden group">
        
        {/* Ambient Lighting Background Accents */}
        <div className="absolute -top-32 -left-32 size-[28rem] rounded-full bg-amber-500/15 blur-3xl pointer-events-none" />
        <div className="absolute -bottom-32 -right-32 size-[28rem] rounded-full bg-rose-500/15 blur-3xl pointer-events-none" />
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-white/10 pb-6 relative z-10">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full bg-amber-400/15 border border-amber-400/30 px-3.5 py-1 text-xs font-black uppercase tracking-wider text-amber-300 mb-2">
              <Sparkles className="size-3.5" />
              <span>Real-Time WebGL 3D Studio</span>
            </div>
            <h2 className="font-nimbus text-3xl sm:text-4xl lg:text-5xl text-white uppercase tracking-tight leading-tight">
              3D Cake Model Atelier
            </h2>
            <p className="text-xs sm:text-sm text-zinc-300 mt-1 max-w-xl font-medium">
              Explore authentic 3D GLB models of our handcrafted small-batch cakes. Drag in 360° to inspect cake layers, mirror ganache drips, and gold truffles.
            </p>
          </div>

          {/* Quick Stats Pill */}
          <div className="flex items-center gap-2 self-start lg:self-auto bg-black/40 backdrop-blur-md px-4 py-2 rounded-2xl border border-white/10">
            <Compass className="size-4 text-amber-400 animate-pulse" />
            <span className="text-xs font-mono font-bold text-amber-200">
              360° Free Orbit & Zoom Enabled
            </span>
          </div>
        </div>

        {/* 2. Main Studio Canvas & Interactive Sidebar */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-stretch mt-6 relative z-10">
          
          {/* Left / Center 3D WebGL Canvas Area (7 Cols) */}
          <div className="lg:col-span-7 flex flex-col items-center justify-center relative min-h-[380px] sm:min-h-[460px] lg:min-h-[520px] rounded-3xl overflow-hidden border-2 border-white/15 bg-black/50 backdrop-blur-md shadow-2xl">
            ...
            {/* Loading Overlay */}
            {loading && (
              <div className="absolute inset-0 bg-[#180E08]/85 backdrop-blur-md flex flex-col items-center justify-center gap-3 z-30 pointer-events-none">
                <div className="size-12 rounded-full border-3 border-amber-400 border-t-transparent animate-spin" />
                <p className="font-blogh uppercase tracking-wider text-sm font-bold text-amber-300">
                  Loading 3D Cake Model… {loadingProgress > 0 ? `${loadingProgress}%` : ""}
                </p>
                <p className="text-xs text-zinc-400 font-medium">
                  Preparing high-poly meshes, chocolate textures & lighting
                </p>
              </div>
            )}
            ...
            {/* Error Overlay */}
            {loadError && (
              <div className="absolute inset-0 bg-[#180E08]/90 flex flex-col items-center justify-center gap-2 z-30 p-6 text-center">
                <Info className="size-8 text-amber-400" />
                <p className="text-sm font-bold text-white">{loadError}</p>
                <Button size="sm" onClick={() => window.location.reload()} className="mt-2 rounded-full">
                  Retry Loading
                </Button>
              </div>
            )}
            ...
            <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none z-20">
              <div className="pointer-events-auto flex items-center gap-1.5 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/15 text-[11px] font-bold text-amber-200 shadow-lg">
              ...
              <div className="pointer-events-auto flex items-center gap-1 bg-black/60 backdrop-blur-md p-1 rounded-full border border-white/15 shadow-lg">
                <button
                  ...
                  className={`p-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
                    autoRotate ? "bg-amber-400 text-black shadow-xs" : "text-white/70 hover:text-white"
                  }`}
                >
                ...
                <button
                  ...
                  title="Reset Camera"
                  onClick={resetCamera}
                  className="p-1.5 rounded-full text-xs font-bold text-white/70 hover:text-white transition-all cursor-pointer"
                >
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full text-xs font-bold transition-all cursor-pointer ${
                    wireframe ? "bg-amber-400 text-black shadow-xs" : "text-white/70 hover:text-white"
                  }`}
                >
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between pointer-events-none z-20">
              <div className="pointer-events-auto flex items-center gap-1 bg-black/60 backdrop-blur-md p-1 rounded-2xl border border-white/15 shadow-lg text-[11px] font-bold">
                <span className="px-2 text-zinc-400 text-[10px] uppercase font-mono">Lighting:</span>
                {(["warm", "neutral", "dramatic"] as const).map((preset) => (
                  <button
                    key={preset}
                    type="button"
                    onClick={() => setLightingPreset(preset)}
                    className={`px-2.5 py-1 rounded-xl uppercase text-[10px] font-bold transition-all cursor-pointer ${
              ...
              <span className="text-[10px] text-zinc-400 font-mono hidden sm:inline-block pointer-events-none">
                WebGL 2.0 · Three.js
              </span>
            </div>
          ...
          {/* Right Spec Sheet & Model Carousel (5 Cols) */}
          <div className="lg:col-span-5 flex flex-col justify-between space-y-4">
            
            {/* Active Model Description Card */}
            <div className="space-y-3 bg-white/5 border border-white/10 rounded-3xl p-4 sm:p-6 backdrop-blur-sm">
              <div className="flex items-center justify-between gap-2">
                <span className="inline-flex items-center gap-1 rounded-full bg-amber-400/20 text-amber-300 border border-amber-400/30 px-3 py-0.5 text-[10.5px] font-black uppercase tracking-wider">
                  {selectedModel.tag}
                </span>
                <span className="text-[11px] font-mono text-emerald-400 font-bold flex items-center gap-1">
                  <Check className="size-3" /> Live 3D Mesh
                </span>
              </div>

              <h3 className="font-nimbus text-xl sm:text-2xl text-white leading-tight">
                {selectedModel.name}
              </h3>
              ...
              <div className="grid grid-cols-2 gap-2 pt-2">
                <div className="rounded-2xl bg-white/5 border border-white/10 p-2.5 text-center">
                  <p className="text-[9.5px] uppercase font-bold text-zinc-400">Calories / Slice</p>
                  <p className="font-sans font-black text-lg text-amber-300 mt-0.5 tracking-tight">
                    {selectedModel.calories}
                  </p>
                </div>
                <div className="rounded-2xl bg-white/5 border border-white/10 p-2.5 text-center">
                  <p className="text-[9.5px] uppercase font-bold text-zinc-400">Clean Protein</p>
                  <p className="font-sans font-black text-lg text-rose-300 mt-0.5 tracking-tight">
                    {selectedModel.protein}
                  </p>
                </div>
                <div className="rounded-2xl bg-white/5 border border-white/10 p-2.5 text-center">
                  <p className="text-[9.5px] uppercase font-bold text-zinc-400">Refined Sugars</p>
                  <p className="font-sans font-black text-lg text-emerald-300 mt-0.5 tracking-tight">
                    {selectedModel.sugar}
                  </p>
                </div>
                <div className="rounded-2xl bg-white/5 border border-white/10 p-2.5 text-center">
                  <p className="text-[9.5px] uppercase font-bold text-zinc-400">Dairy Butter</p>
                  <p className="font-sans font-black text-lg text-amber-200 mt-0.5 tracking-tight">
                ...
                {BAKERY_3D_MODELS.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full rounded-2xl p-2.5 border transition-all flex items-center justify-between gap-3 text-left ${
                        isCurrent
                    ...
                    >
                      <div className="flex items-center gap-3 min-w-0">
                        <div className="size-10 rounded-xl overflow-hidden shrink-0 border border-white/20 bg-black/40">
                          <img src={item.thumbnail} alt={item.name} className="size-full object-cover" />
                        </div>
                        <div className="min-w-0">
                          <p className="text-xs font-bold truncate">{item.name}</p>
                          <p className={`text-[10px] truncate ${isCurrent ? "text-black/80" : "text-zinc-400"}`}>
                            {item.badge}
                          </p>
                        </div>
                      </div>
                      ...
                      {isCurrent ? (
                        <span className="shrink-0 size-6 rounded-full bg-black text-amber-300 flex items-center justify-center">
