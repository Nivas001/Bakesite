...
> & {
  ...
  rainbow?: boolean;
  /** Optional icon to display on the left side (e.g. Search icon) */
  icon?: React.ReactNode;
  /** Show a submit button with an arrow icon on the right side */
  submitButton?: boolean;
  /** Show a quick clear button when there is input value */
  clearable?: boolean;
  onClear?: () => void;
  /**
...
const frontSizeWithButton: Record<MagicInputSize, string> = {
  sm: "py-[var(--button-py-sm)] pl-[var(--button-px-sm)] pr-[2.25rem] text-[length:var(--button-text-sm)] leading-[var(--button-leading-sm)]",
  md: "py-[var(--button-py-md)] pl-[var(--button-px-md)] pr-[2.75rem] text-[length:var(--button-text-md)] leading-[var(--button-leading-md)]",
  lg: "py-[var(--button-py-lg)] pl-[var(--button-px-lg)] pr-[3.5rem] text-[length:var(--button-text-lg)] leading-[var(--button-leading-lg)]",
};

const frontSizeWithIcon: Record<MagicInputSize, string> = {
  sm: "py-[var(--button-py-sm)] pl-[2.2rem] pr-[var(--button-px-sm)] text-[length:var(--button-text-sm)] leading-[var(--button-leading-sm)]",
  md: "py-[var(--button-py-md)] pl-[2.6rem] pr-[var(--button-px-md)] text-[length:var(--button-text-md)] leading-[var(--button-leading-md)]",
  lg: "py-[var(--button-py-lg)] pl-[3.2rem] pr-[var(--button-px-lg)] text-[length:var(--button-text-lg)] leading-[var(--button-leading-lg)]",
};

const frontSizeWithIconAndButton: Record<MagicInputSize, string> = {
  sm: "py-[var(--button-py-sm)] pl-[2.2rem] pr-[2.25rem] text-[length:var(--button-text-sm)] leading-[var(--button-leading-sm)]",
  md: "py-[var(--button-py-md)] pl-[2.6rem] pr-[2.75rem] text-[length:var(--button-text-md)] leading-[var(--button-leading-md)]",
  lg: "py-[var(--button-py-lg)] pl-[3.2rem] pr-[3.5rem] text-[length:var(--button-text-lg)] leading-[var(--button-leading-lg)]",
};
...
export const MagicInput = React.forwardRef<HTMLInputElement, MagicInputProps>(
  (
    ...
    const hasValue = Boolean(value || innerRef.current?.value);
    const showClear = clearable && hasValue && status === "idle";
    const showButton = submitButton || onSubmit != null || showClear;
    ...
    if (disabled) {
      ...
      shadowClass = `absolute inset-0 rounded-2xl translate-y-[5px] opacity-75 blur-[12px] [background-color:transparent] [background-image:linear-gradient(90deg,var(--magic-fill),var(--magic-fill))] bg-no-repeat [will-change:translate] motion-reduce:[transition:none] motion-reduce:animate-none ${fillTransition} ${statusFill}`;
      const edgeTransition = armed
        ? "[transition:background-size_250ms_ease]"
        : "[transition:opacity_250ms_ease]";
      edgeClass = `absolute inset-0 rounded-2xl opacity-100 [background-color:color-mix(in_srgb,var(--foreground)_14%,transparent)] [background-image:linear-gradient(90deg,var(--magic-fill),var(--magic-fill))] bg-no-repeat motion-reduce:[transition:none] motion-reduce:animate-none ${edgeTransition} ${statusFill}`;
    } else {
      const shadowDepth =
        depth === "always"
          ? "opacity-100 translate-y-[3px] group-focus-within:translate-y-[5px]"
      ...
      const shadowRainbow = rainbow
        ? `${RAINBOW_FOCUS_FILL} group-focus-within:blur-[10px] group-focus-within:opacity-75`
        : "";
    ...
    const frontTransform = disabled
      ...
      : hasStatus
        ? "-translate-y-[3px]"
        : depth === "always"
          ? "-translate-y-[3px] group-focus-within:-translate-y-[4px] group-focus-within:[transition:translate_250ms_cubic-bezier(0.3,0.7,0.4,1.5)]"
          : "translate-y-0 group-focus-within:-translate-y-[3px] group-focus-within:[transition:translate_250ms_cubic-bezier(0.3,0.7,0.4,1.5)]";
    const frontText =
      isLoading || isSuccess
    ...
    let paddingClass = frontSize[size];
    if (icon && showButton) {
      paddingClass = frontSizeWithIconAndButton[size];
    } else if (icon) {
      paddingClass = frontSizeWithIcon[size];
    } else if (showButton) {
      paddingClass = frontSizeWithButton[size];
    }

    const frontClass = `relative block w-full box-border rounded-2xl border border-border/80 bg-card/95 backdrop-blur-md outline-none [font:inherit] [will-change:translate] [transition:translate_600ms_cubic-bezier(0.3,0.7,0.4,1),border-color_200ms_ease,box-shadow_200ms_ease] motion-reduce:[transition:none] group-focus-within:border-transparent group-focus-within:shadow-xs disabled:cursor-not-allowed disabled:opacity-50 ${frontText} ${frontTransform} ${paddingClass}`;
    ...
    const submitClass = `absolute top-[4px] right-[4px] bottom-[4px] z-[2] inline-flex aspect-square cursor-pointer items-center justify-center rounded-xl border-none p-0 text-[1rem] leading-none [transition:translate_600ms_cubic-bezier(0.3,0.7,0.4,1),filter_200ms_ease,background-color_200ms_ease] [-webkit-tap-highlight-color:transparent] active:scale-95 focus-visible:[outline:2px_solid_var(--ring)] focus-visible:[outline-offset:2px] disabled:cursor-not-allowed disabled:opacity-50 motion-reduce:[transition:none] ${submitColor} ${submitTransform}`;
    ...
    return (
      <div
        ...
        className={`group relative inline-block rounded-2xl [-webkit-tap-highlight-color:transparent] ${disabled ? "cursor-not-allowed" : ""} ${className ?? ""}`}
        style={
          {
            ...style,
      ...
      >
        ...
        {showButton ? (
          <button
            ...
            aria-label={showClear ? "Clear search" : submitLabel}
            disabled={disabled}
            onClick={handleActionClick}
          >
            {showClear ? (
              <span className="grid place-items-center" data-icon="clear" aria-hidden>
                <XIcon />
              </span>
            ) : (
              <>
